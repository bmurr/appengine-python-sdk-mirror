...
class ModuleOverrideImportHookTest(unittest.TestCase):
  ...
  def tearDown(self):
    ...
    distutils_modules = [
        module for module in sys.modules if module.startswith('distutils')
    ]
  ...
  def test_load_builtin_no_pass_through(self):
    ...
    thread = self.hook.load_module('thread')
    self.assertIsInstance(thread, types.ModuleType)
    self.assertItemsEqual(['__doc__', '__name__', '__package__', '__loader__'],
                          dir(thread))
    self.assertEqual(self.hook, thread.__loader__)
...
class PathOverrideImportHookTest(unittest.TestCase):
  ...
  def test_module_success(self):
    ...
    self.assertEqual(
        hooked_urllib.__file__.replace('.pyc', '.py'),
...
class PathRestrictingImportHookTest(unittest.TestCase):
  ...
  def test_accessible(self):
    imp.find_module('bar', ['foo']).AndReturn(
        (None, 'foo/bar.py', (None, None, imp.PY_SOURCE)))
    stubs.FakeFile.is_file_accessible('foo/bar.py').AndReturn(
  ...
  def test_c_module_accessible(self):
    imp.find_module('bar', ['foo']).AndReturn(
        (None, 'foo/bar.so', (None, None, imp.C_EXTENSION)))
    stubs.FakeFile.is_file_accessible('foo/bar.so').AndReturn(
        stubs.FakeFile.Visibility.OK)
    self.mox.ReplayAll()
    self.assertIsNone(self.hook.find_module('foo.bar', ['foo']))
  ...
  def test_compiled_python_accessible(self):
    imp.find_module('bar', ['foo']).AndReturn(
        (None, 'foo/bar.pyc', (None, None, imp.PY_COMPILED)))
    stubs.FakeFile.is_file_accessible('foo/bar.pyc').AndReturn(
        stubs.FakeFile.Visibility.OK)
    self.mox.ReplayAll()
    self.assertIsNone(self.hook.find_module('foo.bar', ['foo']))
  ...
  def test_c_builtin(self):
    imp.find_module('bar', ['foo']).AndReturn(
        (None, 'bar', (None, None, imp.C_BUILTIN)))
    self.mox.ReplayAll()
    self.assertIsNone(self.hook.find_module('foo.bar', ['foo']))

  def test_py_frozen(self):
    imp.find_module('bar', ['foo']).AndReturn(
        (None, 'bar', (None, None, imp.PY_FROZEN)))
    self.mox.ReplayAll()
    self.assertIsNone(self.hook.find_module('foo.bar', ['foo']))

  def test_enabled_c_library(self):
    imp.find_module('lxmla', ['foo']).AndReturn(
        (None, 'lxmla.py', (None, None, imp.PY_SOURCE)))
    stubs.FakeFile.is_file_accessible('lxmla.py').AndReturn(
        stubs.FakeFile.Visibility.PATH_BLOCK)
    self.mox.ReplayAll()
  ...
  def test_find_module(self):
    self.assertIsInstance(
        sandbox.PyCryptoRandomImportHook.find_module(
            'Crypto.Random.OSRNG.posix'), sandbox.PyCryptoRandomImportHook)
    self.assertIsNone(
