...
class _SingleAddressWsgiServerTest(unittest.TestCase):
  ...
  def test_serve(self):
    result = six.moves.urllib.request.urlopen(
        'http://localhost:%d/foo?bar=baz' % self.server.port)
    body = result.read()
  ...
  def wsgi_application(self, environ, start_response):
    ...
    for key, value in serializable_environ.items():
      if isinstance(value, six.binary_type):
        serializable_environ[key] = six.ensure_text(value)
...
class SharedCherryPyThreadPoolTest(unittest.TestCase):
  ...
  def test_stop_with_connections(self):
    ...
    self.mox.ReplayAll()
    with mock.patch.object(self.thread_pool,
                           '_shutdown_connection') as shutdown_mock:
      self.thread_pool.stop(1)
    shutdown_mock.assert_called_with(connection)
    self.mox.VerifyAll()
...
class SelectThreadTest(unittest.TestCase):
  ...
  def test_add_socket(self):
    ...
    self.assertEqual(file_descriptor_to_callback_copy,
                     file_descriptor_to_callback)
    self.assertEqual(
        frozenset([s.fileno()]), self.select_thread._file_descriptors)
  ...
  def test_remove_socket(self):
    ...
    self.assertEqual(file_descriptor_to_callback_copy,
                     file_descriptor_to_callback)
    self.assertEqual(
        frozenset([s2.fileno()]), self.select_thread._file_descriptors)
...
class WsgiServerStartupTest(unittest.TestCase):
  ...
  def test_start_some_fail_to_bind(self):
    ...
    self.server.start()
    self.mox.VerifyAll()
    six.assertCountEqual(self, [starting_server, another_starting_server],
  ...
  def test_remove_duplicates(self):
    ...
    wsgi_server._SingleAddressWsgiServer(('127.0.0.1', 123),
                                         None).AndReturn(foo_server)
    foo_server.start()
    wsgi_server._SingleAddressWsgiServer(('::1', 123),
                                         None).AndReturn(foo2_server)
    foo2_server.start()
  ...
  def test_quit(self):
    running_server = self.mox.CreateMock(wsgi_server._SingleAddressWsgiServer)
...
class WsgiServerPort0StartupTest(unittest.TestCase):
  ...
  def test_basic_behavior(self):
    ...
    self.server.start()
    self.mox.VerifyAll()
    six.assertCountEqual(self, [inet4_server, inet6_server],
                         self.server._servers)
  ...
  def test_retry_eaddrinuse(self):
    ...
    # Retry
    wsgi_server._SingleAddressWsgiServer(('127.0.0.1', 0),
                                         None).AndReturn(inet4_server_retry)
    inet4_server_retry.start()
    inet4_server_retry.port = 456
    wsgi_server._SingleAddressWsgiServer(('::1', 456),
                                         None).AndReturn(inet6_server_retry)
    inet6_server_retry.start()
    self.mox.ReplayAll()
    self.server.start()
    self.mox.VerifyAll()
    six.assertCountEqual(self, [inet4_server_retry, inet6_server_retry],
                         self.server._servers)
  ...
  def test_retry_limited(self):
    ...
    for offset, (inet4_server,
                 inet6_server) in enumerate(zip(inet4_servers, inet6_servers)):
      wsgi_server._SingleAddressWsgiServer(('127.0.0.1', 0),
                                           None).AndReturn(inet4_server)
      inet4_server.start()
  ...
  def test_ignore_other_errors(self):
    ...
    self.server.start()
    self.mox.VerifyAll()
    six.assertCountEqual(self, [inet4_server], self.server._servers)
...
class _SingleAddressWsgiServerStartupTest(unittest.TestCase):
  ...
  def test_start_port_in_use(self):
    ...
    proto = object()
    socket.getaddrinfo('localhost', 0, socket.AF_UNSPEC, socket.SOCK_STREAM, 0,
                       socket.AI_PASSIVE).AndReturn([(af, socktype, proto, None,
                                                      None)])
  ...
  def test_start(self):
    ...
    wsgi_server._SELECT_THREAD.add_socket(
        mox.IsA(socket.socket), self.server.tick)
    self.mox.ReplayAll()
    self.server.start()
    self.mox.VerifyAll()
...
class WsgiHostCheckTest(unittest.TestCase):
  ...
  def add_host_check_with_whitelisted_hosts(self, whitelisted_hosts):
    ...
    self.server.set_app(
        wsgi_server.WsgiHostCheck(whitelisted_hosts, wsgi_application))

  def send_http_10_request_as_host(self, host):

    class HTTPv1Connection(six.moves.http_client.HTTPConnection):
      _http_vsn = 10
      _http_vsn_str = 'HTTP/1.0'

    class HTTPv1Handler(six.moves.urllib.request.HTTPHandler):

      def http_open(self, req):
        return self.do_open(HTTPv1Connection, req)

    http_10_opener = six.moves.urllib.request.build_opener(HTTPv1Handler)
    request = six.moves.urllib.request.Request(
        'http://localhost:%d' % self.server.port, None, {'Host': host})
    return http_10_opener.open(request)

  def send_request_as_host(self, host):
    request = six.moves.urllib.request.Request(
        'http://localhost:%d' % self.server.port, None, {'Host': host})
    return six.moves.urllib.request.urlopen(request)
  ...
  def assert_host_check_passes_with_request_host(self, host, use_http_10=False):
    ...
    self.assertEqual(six.b('Host check passed'), response.read())
  ...
  def test_whitelisted_wildcard_host_passes_host_check(self):
    self.add_host_check_with_whitelisted_hosts(
        ['local.dev', '*.example.dev', '**.deep.example'])
