...
def _execute_request(request, use_proto3=False):
  ...
  service_methods = remote_api_services.STUB_SERVICE_PB_MAP.get(service, {})
...
def _verify_wormhole_usage(request_info, request):
  ...
  try:
    app_engine_apis = request_info.get_app_engine_apis(request.request_id)
  except KeyError:
    # We assume that the request is valid here to handle the case where java
    # runtime makes an api request with an invalid request_id
    # (See RemoteApiDelegate.MakeSyncCall).
...
class APIServer(wsgi_server.WsgiServer):
    ...
    class CallHandler(grpc_service_pb2.BetaCallHandlerServicer):
      ...
      def HandleCall(self, request, context):
        ...
        try:
          ...
          response.response = api_response.SerializeToString()
  ...
  def _handle_POST(self, environ, start_response):
    ...
    try:
      request = remote_api_bytes_pb2.Request()
      ...
      if isinstance(service_stub, datastore_grpc_stub.DatastoreGrpcStub):
        ...
        api_response = _execute_request(request).SerializeToString()
      ...
      if isinstance(e, apiproxy_errors.ApplicationError):
        ...
        application_error.code = e.application_error
        application_error.detail = e.error_detail
      else:
      ...
      logging.log(level, 'Exception while handling %s.%s()\n%s',
                  request.service_name, request.method, traceback.format_exc())
    encoded_response = response.SerializeToString()
    logging.debug('Handled %s.%s in %0.4f', request.service_name,
                  request.method,
  ...
  def _handle_CLEAR(self, environ, start_response):
    ...
    stubs_to_clear = six.moves.urllib.parse.parse_qs(
        environ.get('QUERY_STRING')).get('stub')
