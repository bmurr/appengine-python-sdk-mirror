...
from google.appengine.tools.devappserver2.python.runtime import pdb_sandbox
...
# Logging will be lazy imported in _init_logging().
logging = None
...
if sys.platform.startswith('win'):
  ...
  _WHITE_LIST_C_MODULES.extend(WIN_ONLY_WHITELISTS)
  ...
  def __init__(self, name, whitelist_regex, import_name=None):
    ...
    Args:
      ...
      import_name: The string identifier of the imported package. Eg, for the
        library named 'pycrypto', the imported package is named 'Crypto'.for
        grpcio, the imported package is named 'grpc'.
    """
    self.name = name
...
class PathOverrideImportHook(BaseImportHook):
  ...
  def __init__(self, modules):
    ...
    if 'grpc' in modules:
      # Add paths to libraries that grpc depend on.





      grpc_dep_libraries = ['enum', 'six', 'concurrent.futures',
                            'google.protobuf']
      for dep_lib_name in grpc_dep_libraries:
        dep_lib = __import__(dep_lib_name)
        if hasattr(dep_lib, '__path__'):
          self.extra_sys_paths.extend(dep_lib.__path__)
        else:
          self.extra_sys_paths.extend(dep_lib.__file__)
...
class CModuleImportHook(object):
  ...
  _modules_to_warn = set(WIN_ONLY_WHITELISTS)
  ...
  def find_module(self, fullname, path=None):
    ...
    if (fullname in _WHITE_LIST_C_MODULES or
      ...
      # logging is lazy imported in _init_logging(). Need to check if its None.
      if logging and basename in self._modules_to_warn:
        logging.warning(
            'The module %s is whitelisted for local dev only. If your '
            'application relies on %s, it is likely that it will not function'
            ' properly in production.', basename, basename)
        # Avoid repeated warning about the same whitelisted module.
        self._modules_to_warn.remove(basename)
      return None
...
class PathRestrictingImportHook(object):
  ...
  def find_module(self, fullname, path=None):
    ...
    _, _, file_type = description
    if file_type in self._EXCLUDED_TYPES:
      return None
    if (stubs.FakeFile.is_file_accessible(filename) ==
        stubs.FakeFile.Visibility.OK):
      return None
    if (filename.endswith('.pyc') and
