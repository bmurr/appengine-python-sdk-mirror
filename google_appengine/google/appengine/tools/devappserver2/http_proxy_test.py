...
class HttpProxyTest(wsgi_test_utils.WSGITestCase):
  ...
  def setUp(self):
    ...
    self.mox.StubOutWithMock(six.moves.http_client.HTTPConnection,
                             'getresponse')
  ...
  def test_handle_get(self):
    response = FakeHttpResponse(200, 'OK', [('Foo', 'a'), ('Foo', 'b'),
                                            ('Var', 'c')], 'response')
  ...
  def test_handle_post(self):
    ...
    environ = {
        'HTTP_HEADER': 'value',
        'PATH_INFO': '/post',
        'wsgi.input': six.StringIO('post data'),
        'CONTENT_LENGTH': '9',
  ...
  def test_handle_ssl(self):
    response = FakeHttpResponse(200, 'OK', [('Foo', 'a'), ('Foo', 'b'),
                                            ('Var', 'c')], 'response')
    login.get_user_info(None).AndReturn(('', False, ''))
    six.moves.http_client.HTTPConnection.connect()  # pylint: disable=no-value-for-parameter
    six.moves.http_client.HTTPConnection.request(
        'GET', '/get%20request?key=value', None, {
            'HEADER': 'value',
            http_runtime_constants.REQUEST_ID_HEADER: 'request id',
    ...
    six.moves.http_client.HTTPConnection.getresponse().AndReturn(response)  # pylint: disable=no-value-for-parameter
    six.moves.http_client.HTTPConnection.close()  # pylint: disable=no-value-for-parameter
    environ = {
        'HTTP_HEADER': 'value',
        'PATH_INFO': '/get request',
        'QUERY_STRING': 'key=value',
  ...
  def test_http_response_early_failure(self):
    ...
    six.moves.http_client.HTTPConnection.getresponse().AndRaise(
